 ...
 * Fully on-device — no API key, no network. The app stays useful on any phone
 * before setup and when the backend is unreachable.
 * Returns the reply text, or null when nothing matched (caller falls through
 * to the LLM agent).
 */
object OfflineIntents {

    suspend fun tryHandle(activity: AppCompatActivity, tools: DeviceTools, text: String): String? {
        ...
        if (flash != null) {
            ...
            return tools.runArgs(activity, "toggle_flashlight", JSONObject().put("state", if (on) "on" else "off"))
        }
        ...
        if (Regex("""\btime\b.*\?|what.*time|current time|the time\b|\bdate\b|what day|today'?s date""").containsMatchIn(t)) {
            return tools.runArgs(activity, "get_time", JSONObject())
        }
        // battery: "battery?", "battery status/level"
        if (t.contains("battery")) {
            return tools.runArgs(activity, "get_battery", JSONObject())
        }
        ...
        if (open != null) {
            ...
            if (app.isNotEmpty()) return tools.runArgs(activity, "open_app", JSONObject().put("app", app))
        }
        // memory is local too: "remember …" / "forget …"
        val remember = Regex("""^remember\s+(.+)$""").find(text.trim())
        if (remember != null) {
            return tools.runArgs(activity, "remember", JSONObject().put("fact", remember.groupValues[1].trim()))
        }
        val forget = Regex("""^forget\s+(.+)$""").find(text.trim())
        if (forget != null) {
            return tools.runArgs(activity, "forget", JSONObject().put("fact", forget.groupValues[1].trim()))
        }
        return null
    }
}
